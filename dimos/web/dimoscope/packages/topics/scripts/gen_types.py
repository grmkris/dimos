...
def generate(path: Path) -> str:
    ...
    target = next(
        (o for o in vars(mod).values()
         if inspect.isclass(o) and issubclass(o, Module) and o is not Module
    ...
    imports = (
        f'import type {{ {", ".join(sorted(pkgs))} }} from "@dimos/msgs";\n\n' if pkgs else ""
    )
    return (
...
def main() -> None:
    ap = argparse.ArgumentParser(description="Generate DimosTopics/DimosCommands from a dimos blueprint.")
