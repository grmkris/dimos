...
DIMOS_TRANSPORT=lcm BENCH_HZ=100 uv run python scenarios/bench.py
```
...
## 2. Mac — point the browser at the VPS (zero new code)

```bash
# real browser, all 5 transports (Chrome):
deno task app                                   # Vite on :5173 (the app is served from your Mac)
#   open  http://localhost:5173/?gw=37.60.232.68:8080          → the app
#   open  http://localhost:5173/bench.html?gw=37.60.232.68:8080 → the 5-mechanism bench sweep
#   flip the topbar dropdown: WebSocket · SSE · HTTP poll · WebRTC data · WebTransport
# (or, if app/dist was built on the VPS, just open http://37.60.232.68:8080/ directly — same-origin)
```

## 3. Read it
Record the browser bench's per-mechanism hz/latency/loss over the real path (`/bench.html` copies its
results table as Markdown; results are also on `window.__benchResults`). The real RTT to the VPS region
and which transports stay smooth under real loss is the client-credible result.
...
- the gateway + a single `scenarios/bench.py` source running in **tmux session `dimos-realwan-65329`**
  (serve on `:8080`, WT on `:8443`); source PID in `~/realwan-source.pid`; logs `~/realwan-serve.log` /
  `~/realwan-source.log`.
- **ufw** rules added: `8080/tcp` + `8443/udp` (only these two — other rules e.g. 6001/6002/2222 belong
  to other work, leave them).
- Agent status file `~/DIMOS-BENCH-AGENT-65329a9d.md`.
...
ssh -i ~/.ssh/vps-coolify kristjan@37.60.232.68 '
  ...
  pkill -f "dimos-bench.*gateway"; pkill -f "dimos-bench.*scenarios/bench.py"   # belt-and-suspenders (scoped to ~/dimos-bench)
