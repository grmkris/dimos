"""CI-checkable transport benchmark: start the dimoscope service (the gateway) + bench publisher,
...
def test_bench_lcm() -> None:
    ...
    try:
        ...
        time.sleep(8)  # the gateway cold start (dimos import for egress + zenoh) + publisher warmup
        subprocess.run(
