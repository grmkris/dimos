...
const loss = (
  s: Stat,
) => (s.recv > 1 ? r2(Math.max(0, (1 - s.recv / (s.max - s.min + 1)) * 100)) : 0);
...
const poseLoss = (m: Map<string, Stat>) =>
  r2(POSE.map((t) => loss(get(m, t))).reduce((a, b) => a + b, 0) / 4);
...
if (MODE === "basic") {
  ...
  const imgCap = get(
    await measure(["/bench/img"], DUR, (c) => c.topic("/bench/img").setRateLimit(2)),
    "/bench/img",
  );
  ...
  console.log(
    `→ ${r2(kbps(imgFull) / Math.max(1, kbps(imgCap)))}× less bandwidth, client-driven.\n`,
  );
  ...
  console.log(
    `1 of 4: ${one} kB/s · all 4: ${four} kB/s → ${r2((1 - one / four) * 100)}% reduction\n`,
  );
} else {
  ...
  const cap = await measure(
    [...POSE, "/bench/img"],
    DUR,
    (c) => c.topic("/bench/img").setRateLimit(2),
  );
  ...
  console.log(
    `| full | ${poseLoss(full)} | ${
      r2(
        POSE.map((t) => hz(get(full, t))).reduce((a, b) => a + b, 0),
      )
    } | ${kbps(get(full, "/bench/img"))} |`,
  );
  console.log(
    `| capped(2Hz) | ${poseLoss(cap)} | ${
      r2(
        POSE.map((t) => hz(get(cap, t))).reduce((a, b) => a + b, 0),
      )
    } | ${kbps(get(cap, "/bench/img"))} |`,
  );
}
Deno.exit(0);
