...
import { createDimosClient } from "../packages/topics/src/index.ts";
import { zenohTs } from "../packages/topics/src/experimental.ts";
import {
  BENCH_SCENARIOS,
  type BenchRow,
  formatMarkdown,
...
for (const scenario of BENCH_SCENARIOS) {
  ...
  const client = createDimosClient({ transport: zenohTs({ discoveryKey: "" }) });
  await client.connect(TS_URL);
