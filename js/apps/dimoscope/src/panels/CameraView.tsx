import { useTopics, useVideo } from "@dimos/react";
...
function pickImage(topics: TopicInfo[]): string | null {
  const prefer = [
    "/dimos/color_image",
    "/color_image",
    "/dimos/camera/image_raw",
    "/image",
  ];
  for (const p of prefer) if (topics.some((t) => t.topic === p)) return p;
...
export function CameraView({
  ...
  const { kind, videoRef, canvasRef, label, active, requested } = useVideo(
    topic,
    {
      mode: effectiveMode,
  ...
  const fullscreen = () => {
    const el = (
      kind === "stream" ? videoRef.current : canvasRef.current
    ) as HTMLElement | null;
      ...
      >
        ...
        >
          ...
          {cv.enabled && kind === "stream" && (
            <span style={{ color: "var(--accent)" }}>
              {" "}
              · ⚠ CV needs webcodecs/jpeg (frames)
            </span>
          )}
          {cv.enabled && kind === "frames" && (
            <span className="muted">
              ...
              {cv.stats.loading
                ? "loading…"
                : `${cv.stats.count} obj · ${cv.stats.infMs}ms`}
            </span>
          )}
        </span>
        <button
