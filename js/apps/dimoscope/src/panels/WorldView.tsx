...
function pick(
  ...
  for (const p of prefer)
    if (topics.some((t) => t.topic === p && t.type === type)) return p;
...
export function WorldView() {
  ...
  const lidarTopic = pick(topics, "sensor_msgs.PointCloud2", [
    "/dimos/lidar",
    "/lidar",
  ]);
  ...
  useEffect(() => {
    ...
    const onWheel = (e: WheelEvent) => {
      ...
      const nS = Math.max(
        MIN_SCALE,
        Math.min(MAX_SCALE, S * Math.exp(-e.deltaY * 0.0015))
      );
  ...
  useEffect(() => {
    ...
    const draw = () => {
      ...
      const wxMin = view.current.cx - cw / 2 / S,
        wxMax = view.current.cx + cw / 2 / S;
      const wyMin = view.current.cy - ch / 2 / S,
        wyMax = view.current.cy + ch / 2 / S;
      ...
      if (p) {
        ...
        const last = t.at(-1);
        if (!last || Math.hypot(last[0] - rx, last[1] - ry) > 0.02)
          t.push([rx, ry]);
        ...
        ctx.beginPath();
        t.forEach(([wx, wy], i) =>
          i ? ctx.lineTo(W(wx), H(wy)) : ctx.moveTo(W(wx), H(wy))
        );
        ctx.stroke();
        ctx.save();
        ...
        ctx.fillText(
          "waiting for spatial topics (pose / map / scan / path)…",
          16,
          26
        );
      }
  ...
  return (
    ...
    >
      ...
      >
        <span>
          WorldView · {title} ·{" "}
          <span className="muted">scroll=zoom drag=pan click=goal</span>
        </span>
        <button
          className="tab"
          onClick={recenter}
