...
import type { BenchRow, DimosClient } from "@dimos/topics";
...
function Bench() {
  ...
  const md = `# dimoscope transport benchmark — in-browser (all 3 transports)\n\n_${stamp} · ${DUR}ms/scenario · end-to-end latency (publish→browser)_\n\n${results
    .map((r) => formatMarkdown(r.label, r.url, DUR, stamp, r.rows))
    .join("\n---\n\n")}`;
  ...
  return (
    <div style={{ padding: 24, maxWidth: 1100, margin: "0 auto" }}>
      ...
      <p className="muted small">
        ...
        (3 servers + bench_publisher on both buses). {DUR}ms/scenario · latency
        is end-to-end (publish→browser), comparable across transports.
      </p>
      <div
        style={{
          display: "flex",
...
createRoot(document.querySelector("#root")!).render(<Bench />);
