...
The thesis: **"DimOS topics in the browser", not "Zenoh in the browser."** A small `Transport` interface + the self-describing 8-byte `@dimos/msgs` codec means the _same_ app and SDK code runs over LCM or Zenoh, and the gateway stays a thin byte-relay.
...
- **[custom-messages-in-the-browser.md](./custom-messages-in-the-browser.md)** — a zero-rebuild design proposal: how a user's _custom_ message type can render in the browser without regenerating/republishing a codec package.
