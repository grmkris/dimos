...
import type { OccupancyGrid } from "@dimos/msgs/nav_msgs";
...
const client = await connect({ reconnect: false, url });
...
const map = client.topic<OccupancyGrid>("/map");
let gotMap = false;
...
setTimeout(() => {
  ...
  console.log(
    ok ? "[smoke] OK ✅ SDK decodes + stats work" : "[smoke] FAIL ❌"
  );
  client.close();
  process.exit(ok ? 0 : 1);
}, 4000);
