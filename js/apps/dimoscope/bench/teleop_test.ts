import type { PoseStamped } from "@dimos/msgs/geometry_msgs";
...
const client = await connect({
  reconnect: false,
  url: process.env.GATEWAY_URL ?? "ws://localhost:8090",
});
let x = 0,
  y = 0;
client
  .topic<PoseStamped>(process.env.ODOM_TOPIC ?? "/odom")
  .subscribeLatest((d) => {
    x = d.pose.position.x;
    y = d.pose.position.y;
  });
...
const x0 = x;
console.log(
  `[teleop] start x=${x0.toFixed(3)} — driving forward 0.5 m/s for ${DRIVE_MS}ms`
);
...
setTimeout(() => {
  ...
  setTimeout(() => {
    ...
    console.log(
      moved > 0.05
        ? "[teleop] OK ✅ robot moved via browser-SDK teleop"
        : "[teleop] no motion ❌"
    );
    client.close();
