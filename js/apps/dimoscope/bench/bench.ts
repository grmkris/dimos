...
import type { BenchRow } from "@dimos/topics";
...
async function buildClient() {
  ...
  return await connect({ url: WS_URL, reconnect: false });
}
...
await Bun.write(
  ...
  JSON.stringify(
    { label: LABEL, url, durMs: DUR, stamp, rows, ondemandSaving: saving },
    null,
    2
  )
);
...
process.exit((rows[0]?.msgs ?? 0) > 0 ? 0 : 1);
