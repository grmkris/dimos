...
import { parseControl } from "@dimos/topics/control";
...
const MAX_LIN = Number(process.env.TELEOP_MAX_LIN ?? 1); // m/s clamp
...
const MAGIC_SHORT = 0x4c_43_30_32; // "LC02" single packet
const MAGIC_LONG = 0x4c_43_30_33; // "LC03" fragment
const FRAG_HDR = 20;
...
function parseChannel(pkt: Uint8Array): { topic: string; type: string } | null {
  ...
  let end = 8;
  while (end < pkt.length && pkt[end] !== 0) {
    end++;
  }
  if (end >= pkt.length) {
    return null;
...
// ── LCM → browser ───────────────────────────────────────────────────────────
const udp = dgram.createSocket({ reuseAddr: true, type: "udp4" });
udp.on("message", (buf) => {
  rxPackets++;
  ...
  for (const ws of clients) {
    const st = ws.data;
    if (!(st.subs.has("*") || st.subs.has(meta.topic))) {
      continue;
    } // on-demand filter
...
udp.bind(LPORT, () => {
  try {
    ...
    console.error("[lcm] addMembership failed (multicast route?):", error);
  }
...
function publishTwist(linX: number, angZ: number) {
  const t = new geometry_msgs.Twist({
    angular: new geometry_msgs.Vector3({
      x: 0,
      y: 0,
      z: clamp(angZ, -MAX_ANG, MAX_ANG),
    }),
    linear: new geometry_msgs.Vector3({
      x: clamp(linX, -MAX_LIN, MAX_LIN),
      y: 0,
      z: 0,
    }),
  });
...
function publishGoal(x: number, y: number, z: number) {
  ...
  udp.send(
    encodePacket("/clicked_point#geometry_msgs.PointStamped", ps),
    LPORT,
    MCAST
  );
}
...
Bun.serve<ClientState>({
  ...
  websocket: {
    ...
    message(ws, raw) {
      ...
      if (!m) return; // malformed / unknown control message
      switch (m.op) {
        case "subscribe":
          st.subs.add(m.topic);
          if (m.maxHz !== undefined) st.rate.set(m.topic, m.maxHz);
          break;
        case "unsubscribe":
        ...
        case "list":
          ...
          break;
        case "teleop":
          publishTwist(m.linearX, m.angularZ);
          armDeadman(st, m.ttlMs ?? DEFAULT_TTL);
          break;
        case "stop":
          if (st.teleopTimer) clearTimeout(st.teleopTimer);
          publishTwist(0, 0);
    ...
    open(ws) {
      clients.add(ws);
      ws.send(
        JSON.stringify({ op: "hello", topics: topicList(), label: "Bun↔LCM" })
      );
