...
it has never seen — that's what makes a _generic_ viewer possible. `@dimos/msgs`
...
_receive_ and _send_ UDP packets. `node:dgram` does multicast UDP in Bun/Node,
...
> **LeRobot bridge:** a dimos _Module_ (`In[T]`/`Out[T]` ports) is the unit of
...
in the registry — that's the difference between a demo and a _framework_.
