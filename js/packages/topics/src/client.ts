...
export const createDimosClient = (deps: DimosClientDeps): DimosClient => {
  ...
  function topic<T = unknown>(name: string): Topic<T> {
    ...
    if (!t) {
      t = createTopic<T>({
        ...
        wiring: {
          subscribe: (topicName, maxHz) =>
            transport.subscribe(topicName, maxHz),
  ...
  function listTopics(): TopicInfo[] {
    return [...topicsMap]
      .filter(
        ([name, type]) =>
          type !== "?" && name.startsWith("/") && !name.includes("/rpc/")
      )
      .map(([name, type]) => ({ topic: name, type }));
  }
  ...
  // ── wiring: subscribe to the transport once at construction (after `client`
  // exists, so the onStatus closure can set client.status) ─────────────────────
...
export async function connect(opts: ConnectOpts = {}): Promise<DimosClient> {
  const transport =
    ...
    createGatewayWsTransport({
      url: opts.url ?? "ws://localhost:8090",
