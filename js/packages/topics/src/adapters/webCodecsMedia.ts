...
export const createWebCodecsMedia = (
  deps: WebCodecsMediaDeps
): MediaChannel => {
  ...
  let frameCb:
    | ((id: string, frame: VideoFrame | ImageBitmap, m: VideoMeta) => void)
    | undefined;
  ...
  function configure(topic: string, c = "avc1.42E01F"): void {
  ...
  function subscribe(streamId: string): void {
    ...
    if (ws?.readyState === WebSocket.OPEN)
      send({ op: "webcodecs-start", topic: streamId });
  }
  ...
  return {
    ...
    onFrame(
      cb: (id: string, frame: VideoFrame | ImageBitmap, m: VideoMeta) => void
    ): void {
      frameCb = cb;
    },
