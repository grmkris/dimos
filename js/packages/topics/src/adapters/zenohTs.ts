...
const topicToKey = (topic: string) => `${topic.replace(/^\//u, "")}/**`;
...
export const createZenohTsTransport = (deps: ZenohTsDeps): Transport => {
  ...
  function subscribe(topic: string, maxHz?: number) {
    ...
    session
      .declareSubscriber(topicToKey(topic), {
        handler: (s: any) => onSampleZ(topic, s),
      })
      .then((sub: any) => {
  ...
  function ctrl(): WebSocket | undefined {
    ...
    if (control && (st === WebSocket.OPEN || st === WebSocket.CONNECTING))
      return control;
    ...
    return new Promise((resolve, reject) => {
      ...
      setTimeout(() => {
        if (pending.delete(id))
          reject(new Error(`rpc ${target}/${method} timed out`));
      }, 8000);
    });
  }
  ...
  function close() {
    for (const sub of subs.values())
      if (sub !== "pending") sub.undeclare?.().catch(() => {});
    subs.clear();
    control?.close();
    session?.close?.().catch?.(() => {});
    statusCb?.("closed");
