import { connect, selectMediaChannel } from "@dimos/topics";
import type {
  CommandInfo,
  DimosClient,
  MediaChannel,
...
export function DimosProvider({
  ...
  const list = useMemo<ServerOpt[]>(
    () =>
      servers ??
      (url ? [{ id: url, label: url, connect: () => connect({ url }) }] : []),
    [servers, url]
  );
  ...
  return (
    <Ctx.Provider
      value={{ client, status, servers: list, activeId, setActiveId }}
    >
      {children}
    </Ctx.Provider>
  );
  ...
  for (let y = 0; y < h; y++) {
    for (let x = 0; x < w; x++) {
      ...
      if (ch === 1) {
        ...
        out[o] = data[i + (bgr ? 2 : 0)] ?? 0;
        out[o + 1] = data[i + 1] ?? 0;
        out[o + 2] = data[i + (bgr ? 0 : 2)] ?? 0;
        out[o + 3] = ch === 4 ? (data[i + 3] ?? 0) : 255;
      }
    }
  }
  ...
  useEffect(() => {
    ...
    const loop = (ts: number) => {
      ...
      if (enc === "jpeg" || enc === "jpg") {
        ...
        lastDraw = ts;
        createImageBitmap(
          new Blob([img.data as BlobPart], { type: "image/jpeg" })
        )
          .then((bmp) => {
    ...
    const wire = (ch: MediaChannel): Promise<void> => {
      ...
      if (ch.caps.output === "stream") {
        ch.onStream((id, stream) => {
          if (alive && id === topic && videoRef.current)
...
function PinnedTopic({
  ...
  const { data, meta } = useTopicLatest(topic, { maxHz: 4 });
  ...
  const preview =
    data == null
      ? ""
      : JSON.stringify(data, (_k: string, v: unknown) =>
          typeof v === "bigint"
            ? v.toString()
    ...
    >
      <div style={{ display: "flex", alignItems: "center", gap: 8 }}>
        ...
        <span
          ...
          title="message rate · size of the last message"
        >
